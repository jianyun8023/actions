...
<body>
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="form-group">
                <label>服务器地址</label>
                <input type="text" id="server" placeholder="例如: vpn.example.com:4500">
            </div>
            <div class="form-group">
                <label>用户名</label>
    ...
    <script>
        ...
        async function refreshStatus() {
            ...
            if (data.connected) {
                ...
                info.innerHTML = `
                    ...
                    <div class="info-row"><span class="info-label">内网IP</span><span class="info-value">${data.privateIp || '-'}</span></div>
        ...
        async function manualConnect() {
            ...
            log('手动登录中...');
            const data = await api('connect', { server, username, password });
            if (data.success) {
                toast('登录成功');
                log('手动登录成功');
            } else {
...
case "$ACTION" in
    status)
        ...
        if [ -f "$VPN_CMD" ]; then
            ...
            if echo "$OUTPUT" | grep -q "Login User:"; then
                ...
                PRIVATE_IP=$(echo "$OUTPUT" | grep "Private IP:" | sed 's/.*Private IP: \([^ ]*\).*/\1/')
                ENCRYPTION=$(echo "$OUTPUT" | grep "Encryption:" | cut -d':' -f2 | xargs)
                echo "{\"connected\":true,\"user\":\"$USER\",\"server\":\"$SERVER\",\"privateIp\":\"$PRIVATE_IP\",\"encryption\":\"$ENCRYPTION\"}"
            else
    ...
    connect)
        ...
        # 注意：实际的 nationconnect 可能需要交互式输入
        # 这里尝试通过 expect 或管道方式处理
        echo '{"success":false,"error":"手动登录需要交互式操作，请使用快速连接或配置文件"}'
        ;;
